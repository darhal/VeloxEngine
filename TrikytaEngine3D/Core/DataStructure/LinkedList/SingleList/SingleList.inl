...
FORCEINLINE SingleList<T, Alloc_t>::SingleList() : m_Allocator(sizeof(Node), BLOCK_NUM, false), m_Head(NULL)
{
}
...
static void SingleList<U, A>::EmptyList(SingleList<T, Alloc>& list)
{
...
template<typename U, typename A>
template<typename T, typename Alloc, typename std::enable_if<HAVE_DTOR(T), int>::type>
static void SingleList<U, A>::Destroy(SingleList<T, Alloc>& list)
{
	auto head = list.m_Head;
	while (head != NULL) {
		head->m_Obj.~T(); //Make sure to call the dtor for non pod types!
