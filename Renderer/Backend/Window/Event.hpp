...
{
	enum RENDERER_API mouse_button_t
	{
		Left,
		Right,
		Middle
...
struct RENDERER_API MouseEvent
{
	uint32 X, Y;
	union
	{
...
struct RENDERER_API WindowEvent
{
	union { int X; uint32 Width; };
	union { int Y; uint32 Height; };
};

struct RENDERER_API Event {
	enum event_t {
		TE_UNKNOWN,
		TE_CLOSE,
		TE_RESIZE,
