...
#include <Core/FileSystem/AsyncFile/AsyncFileManager.hpp>
...
int main()
{
	...
	String res_path;
	d.Search("xd", res_path);
	printf("Searched for img in %s and found %s\n", d.GetPath().Buffer(), res_path.Buffer());

	String res_path2;
	if (d.SearchRecursive("handgun_N1.jpg", res_path2)){
		printf("Searched for handgun_N.jpg in %s and found %s\n", d.GetPath().Buffer(), res_path2.Buffer());
	}
	

	/* AsynFileManager manager;
	char ptr[50];
	File f("test.txt", File::Options::OPEN_READ_WRITE);
	manager.BootUp();

	AsyncFileRequest::CallBackFunction rcallback = 
	[](const AsyncFileRequest& rq) -> void
	{
		printf("Reading : %s\n", (const char*)rq.buffer);
	};

	AsyncFileRequest::CallBackFunction wcallback = 
	[](const AsyncFileRequest& rq) -> void
	{
		printf("Written : %s\n", (const char*)rq.buffer);
	};

	manager.template AddRequest<AsyncFileRequest::READ>(&f, (void*)ptr, 50, rcallback);
	usize i = 0;
	usize j = 0;

	const char* arr[4] = {"Write this please :p\n", "And this", " -> ok\n", "Im done!"};

	for (;;){
		printf("zz...\n");

		if (i < 4){
			manager.template AddRequest<AsyncFileRequest::WRITE>(&f, (void*)arr[i], 0, wcallback);
			i++;
		}else{
			manager.template AddRequest<AsyncFileRequest::READ>(&f, (void*)ptr, 50, rcallback);
			j++;
	...
	std::thread t([&](){
		for(;;){
			...
			if (!list.IsEmpty()) {
				...
				int* r = list.Front();
				mtx.unlock();
				printf("[T2] We have recieved r = %d\n", *r);
				fflush(stdout);
				mtx.lock();
				list.Pop();
				mtx.unlock();
...
void BenchmarkList()
{
	printf("*** Start of benchmark sizeof(List) = %lu | sizeof(std::list) = %lu ***\n", sizeof(List<String>), sizeof(std::list<String>));
	{
		auto start = std::chrono::steady_clock::now();
		List<String> test;
		auto end = std::chrono::steady_clock::now();
...
{
	printf("sizeof(std::unordered_map<int, String>) = %lu || sizeof(HashMap<int, String>) = %lu\n", sizeof(std::unordered_map<int, String>), sizeof(HashMap<int, String>));
	{
		/*double avrg = 0.;
