...
vec4 CalculateDirectionalLight(Light light, vec3 normal, vec3 viewDir, float shadow)
{
    ...
    vec3 diffuse  = light.diffuse * diff * vec3(texture(material.diffuse_tex, TexCoords));
    vec3 specular = light.diffuse * spec * vec3(texture(material.specular_tex, TexCoords));
    return ambient + vec4((diffuse + specular) * (1.0 - shadow), 0.0);
}
