...
FORCEINLINE AlignedStackAllocator::AlignedStackAllocator(usize total_size, bool autoInit) : m_Start(NULL), m_Offset(0), m_TotalSize(total_size), m_Marker(0)
{
	if (autoInit) {
		this->InternalInit();
	}
...
FORCEINLINE void* AlignedStackAllocator::Allocate(ssize size, usize alignment)
{
	...
	uintptr_t rawAddress = reinterpret_cast<uintptr_t>(m_Start + m_Offset);
