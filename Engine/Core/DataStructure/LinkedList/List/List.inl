...
FORCEINLINE List<T, Alloc>::List() : m_Head(NULL), m_Tail(NULL), m_Allocator(sizeof(Node), BLOCK_NUM, false)
{
}

template<typename T, typename Alloc>
FORCEINLINE List<T, Alloc>::List(usize nbChunk) : m_Head(NULL), m_Tail(NULL), m_Allocator(sizeof(Node), nbChunk, false)
{
}
...
template<typename T, typename Alloc>
template<usize S>
List<T, Alloc>::List(const T(&objs_array)[S]) : m_Head(NULL), m_Tail(NULL), m_Allocator(sizeof(Node), S)
{
	for (usize i = 0; i < S; i++) {
		this->PushBack(objs_array[i]);
	}
}
